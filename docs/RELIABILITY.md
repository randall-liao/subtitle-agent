...
## Observability
All CLI interactions are logged via `loguru`. During runs, success/failure ratios are aggregated at the very end to present a final report:
- `SuccessCount`: Number of successfully extracted and safely moved subtitle tracks.
- `FailureCount`: Number of videos missing subtitles after agent execution runs.
